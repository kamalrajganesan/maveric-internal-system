...
$(document).ready(function () {
    ...
    fetchAgents().then(() => {
        manageLeadDatatable = $("#leadMasterTbl").DataTable({
            ...
            scrollX: true,
            ajax: {
                url: "./services/lostcall_lead_fetch_all.php",
                dataType: "json",
            },
        });
    });
...
$("#resetAllLeadsBtn").on("click", function () {
...
$("#confirmResetBtn").on("click", function () {
  $.ajax({
    type: "POST",
    url: "./services/lead_call_reset_all.php", // your PHP file
    dataType: "json",
    success: function (response) {
      if (response.success) {
        ...
        $("#resetConfirmModal .modal-body").html(
          `<span class="text-danger">Failed: ${response.message}</span>`
        );
      }
      // Hide footer buttons after showing result
      $("#resetConfirmModal .modal-footer").hide();
      // Reload table
      manageLeadDatatable.ajax.reload(null, true);
    },
    error: function () {
      $("#resetConfirmModal .modal-body").html(
        `<span class="text-danger">Error resetting leads. Please try again.</span>`
      );
      $("#resetConfirmModal .modal-footer").hide();
    }
  });
...
$("#confirmResetBtn").on("click", function() {
    $.ajax({
        type: "POST",
        url: "./services/lead_call_reset_all.php",
        dataType: "json",
       
      success: function(response) {
    $("#resetConfirmModal").modal("hide");
    if (response.success) {
        showToast(response.message, "success"); // ✅ Toast instead of alert
        manageLeadDatatable.ajax.reload(null, true);
    } else {
...
error: function() {
    $("#resetConfirmModal").modal("hide");
    showToast("Error resetting leads. Please try again.", "danger");
}
