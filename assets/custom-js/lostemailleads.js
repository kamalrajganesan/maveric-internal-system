...
$(document).ready(function () {
    ...
    function initializeLeadDatatable() {
        manageLeadDatatable = $('#leadMasterTbl').DataTable({
            ...
            processing: true,
            ajax: {
                url: "./services/lost_email_fetch_all.php",
                type: 'POST',
                data: function(d) {
                    d.assignedOnly = true;
                    d.param = '';
...
$("#resetLeadsBtn").on("click", function() {
...
$("#confirmResetBtn").on("click", function() {
    $.ajax({
        type: "POST",
        url: "./services/lead_email_reset_all.php",
        dataType: "json",
        success: function(response) {
            if (response.success) {
                // Show success message in modal
                $("#resetConfirmModal .modal-body").html(
                    "<div class='text-success fw-bold'>" + response.message + "</div>"
                );
                ...
                $("#resetConfirmModal .modal-body").html(
                    "<div class='text-danger fw-bold'>Failed: " + response.message + "</div>"
                );
            }
        },
        error: function() {
            $("#resetConfirmModal .modal-body").html(
                "<div class='text-danger fw-bold'>Error resetting leads.</div>"
            );
        }
    });
});
