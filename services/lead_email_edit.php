...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $originalFollowUpDateTime = $_POST['originalFollowUpDateTime'] ?? '';
   ...
   if (!empty($_POST['followUpDt'])) {
    ...
    if ($originalFollowUpDateTime) {
        $originalDate = date('Y-m-d', strtotime($originalFollowUpDateTime));
        $newDate      = $dateTime->format('Y-m-d');

        if ($newDate !== $originalDate) {
            // Date changed → use today's time
            $newTime = date('H:i:s');
        } else {
            // Date same → keep original time
            $newTime = date('H:i:s', strtotime($originalFollowUpDateTime));
        }

        $followUpDate = $newDate . ' ' . $newTime;
    } else {
        // No original date → set with current time
        $followUpDate = $dateTime->format('Y-m-d') . ' ' . date('H:i:s');
    }
} elseif ($originalFollowUpDateTime) {
    $followUpDate = $originalFollowUpDateTime;
}
