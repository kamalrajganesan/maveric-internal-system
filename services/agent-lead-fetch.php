...
try {
    ...
    if (!empty($dateRange) && strpos($dateRange, ' to ') !== false) {
        ...
        if (count($startParts) === 3 && count($endParts) === 3) {
            ...
            $dateWhereCall  = "(DATE(lct.created_on) BETWEEN '$startDateEsc' AND '$endDateEsc' OR DATE(lct.updated_on) BETWEEN '$startDateEsc' AND '$endDateEsc')";
            $dateWhereEmail = "(DATE(let.created_on) BETWEEN '$startDateEsc' AND '$endDateEsc' OR DATE(let.updated_on) BETWEEN '$startDateEsc' AND '$endDateEsc')";
        }
    ...
    if (!empty($leadStatus)) {
        ...
        $statusFilterEmail  = ($leadStatus === 'Emailed and Waiting for reply')
            ? " AND let.lead_status = 'Emailed and Waiting for reply'"
            : " AND let.lead_status = '$leadStatusEsc'";
    }
    ...
    SELECT 
        ...
        (
            ...
            WHERE lct.assignee = a.id AND lct.assignee IS NOT NULL 
            AND lct.is_deleted = 0 AND lct.lead_status = 'New'
            " . ($leadType === 'Phone Call' || $leadType === '' ? "AND $dateWhereCall $statusFilterCall" : "AND 1=0") . "
        ) +
        (
            SELECT COUNT(*) FROM lead_email_tracker let
            WHERE let.assignee = a.id AND let.assignee IS NOT NULL 
            AND let.is_deleted = 0 AND let.lead_status = 'New'
            " . ($leadType === 'Email' || $leadType === '' ? "AND $dateWhereEmail $statusFilterEmail" : "AND 1=0") . "
        ) as new_leads,
        ...
        (
            SELECT COUNT(*) FROM lead_call_tracker lct
            WHERE lct.assignee = a.id AND lct.assignee IS NOT NULL 
            AND lct.is_deleted = 0 AND lct.lead_status = 'Following'
            " . ($leadType === 'Phone Call' || $leadType === '' ? "AND $dateWhereCall $statusFilterCall" : "AND 1=0") . "
        ) +
        (
            SELECT COUNT(*) FROM lead_email_tracker let
            WHERE let.assignee = a.id AND let.assignee IS NOT NULL 
            AND let.is_deleted = 0 AND let.lead_status = 'Following'
            " . ($leadType === 'Email' || $leadType === '' ? "AND $dateWhereEmail $statusFilterEmail" : "AND 1=0") . "
        ) as following_up,
        ...
        (
            SELECT COUNT(*) FROM lead_call_tracker lct
            WHERE lct.assignee = a.id AND lct.assignee IS NOT NULL 
            AND lct.is_deleted = 0 AND lct.lead_status = 'Converted'
            " . ($leadType === 'Phone Call' || $leadType === '' ? "AND $dateWhereCall $statusFilterCall" : "AND 1=0") . "
        ) +
        (
            SELECT COUNT(*) FROM lead_email_tracker let
            WHERE let.assignee = a.id AND let.assignee IS NOT NULL 
            AND let.is_deleted = 0 AND let.lead_status = 'Converted'
            " . ($leadType === 'Email' || $leadType === '' ? "AND $dateWhereEmail $statusFilterEmail" : "AND 1=0") . "
        ) as converted,
        ...
        (
            SELECT COUNT(*) FROM lead_call_tracker lct
            WHERE lct.assignee = a.id AND lct.assignee IS NOT NULL 
            AND lct.is_deleted = 0 AND lct.lead_status = 'Contacted'
            " . ($leadType === 'Phone Call' || $leadType === '' ? "AND $dateWhereCall $statusFilterCall" : "AND 1=0") . "
        ) +
        (
            SELECT COUNT(*) FROM lead_email_tracker let
            WHERE let.assignee = a.id AND let.assignee IS NOT NULL 
            AND let.is_deleted = 0 AND let.lead_status IN ('Contacted','Emailed and Waiting for reply')
            " . ($leadType === 'Email' || $leadType === '' ? "AND $dateWhereEmail $statusFilterEmail" : "AND 1=0") . "
        ...
        (
            SELECT COUNT(*) FROM lead_call_tracker lct
            WHERE lct.assignee = a.id AND lct.assignee IS NOT NULL 
            AND lct.is_deleted = 0 AND lct.lead_status = 'Lost'
            " . ($leadType === 'Phone Call' || $leadType === '' ? "AND $dateWhereCall $statusFilterCall" : "AND 1=0") . "
        ) +
        (
            SELECT COUNT(*) FROM lead_email_tracker let
            WHERE let.assignee = a.id AND let.assignee IS NOT NULL 
            AND let.is_deleted = 0 AND let.lead_status = 'Lost'
            " . ($leadType === 'Email' || $leadType === '' ? "AND $dateWhereEmail $statusFilterEmail" : "AND 1=0") . "
        ) as lost,
